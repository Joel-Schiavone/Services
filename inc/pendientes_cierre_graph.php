...
if($usu!=0)
{
	...
	$filter	=	"";
		...
		$sql2	=	"SELECT sta_color1, count(quotations.ID_pto) as count
					...
					WHERE TIMESTAMPDIFF(DAY, TIMESTAMP(quotations.pto_fecIngreso), NOW())<=status.sta_color1 AND
					quotations.ID_sta=status.ID_sta AND
					quotations.ID_sta=14
					$filter";
		...
		$sql21	=	"SELECT sta_color2, count(quotations.ID_pto) as count
					FROM quotations, status
					WHERE TIMESTAMPDIFF(DAY, TIMESTAMP(quotations.pto_fecIngreso), NOW())<=status.sta_color2 AND
					TIMESTAMPDIFF(DAY, TIMESTAMP(quotations.pto_fecIngreso), NOW())>status.sta_color1 AND
					quotations.ID_sta=status.ID_sta AND
					quotations.ID_sta=14
					$filter";
...
?>
<div class="row">
   	<canvas id="cierre" width="400" height="400"></canvas>
<script>
var ctx = document.getElementById("cierre");
