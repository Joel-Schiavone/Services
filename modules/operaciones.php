...
//////////////// CONSULTAS PARA LA TABLA OBRAS
	 ...
	 function busca_obras($ID_obr, $ID_cli, $ID_zon, $obr_nsucursal){
		...
		if($obr_nsucursal == ''){
		 ...
		 $sql		=	"SELECT * FROM obras, clientes, zonas WHERE obras.ID_cli=clientes.ID_cli AND obras.ID_zon=zonas.ID_zon $obra $cliente $zona $sucursal ORDER BY obras.obr_default ASC, clientes.cli_desc ASC, obras.obr_desc ASC";
		 $result	=	mysql_query($sql);
		 return $result;
	 }
...
//////////////// CONSULTAS PARA LA TABLA REGISTRO SERVICIOS
	 ...
	 function registro_servicio(){
		 $sql		=	"SELECT * FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas WHERE registro_servicio.ID_cli=clientes.ID_cli AND registro_servicio.ID_obr=obras.ID_obr AND registro_servicio.ID_sta=status.ID_sta AND registro_servicio.ID_pri=prioridades.ID_pri AND registro_servicio.ID_usu=usuarios.ID_usu AND registro_servicio.ser_tipo='1' ORDER BY clientes.cli_desc ASC, obras.obr_desc ASC";
		 $result	=	mysql_query($sql);
		 return $result;
	 }
	 ...
	 function registro_servicio_filter($ID_cli, $ID_obr, $ser_asig, $ID_sta, $ser_cod){
		...
		if($ser_cod == ''){
		 ...
		 $sql		=	"SELECT * FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas WHERE registro_servicio.ID_cli=clientes.ID_cli AND registro_servicio.ID_obr=obras.ID_obr AND registro_servicio.ID_sta=status.ID_sta AND registro_servicio.ID_pri=prioridades.ID_pri AND registro_servicio.ID_usu=usuarios.ID_usu AND obras.ID_zon=zonas.ID_zon AND registro_servicio.ser_tipo='1' $cliente $obra $asig $codigo $status ORDER BY registro_servicio.ser_cod DESC";
		 $result	=	mysql_query($sql);
		 return $result;
	 }

	 function registro_servicio_filter_usu($ser_asig){
		 $sql		=	"SELECT * FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas WHERE registro_servicio.ID_cli=clientes.ID_cli AND registro_servicio.ID_obr=obras.ID_obr AND registro_servicio.ID_sta=status.ID_sta AND registro_servicio.ID_pri=prioridades.ID_pri AND registro_servicio.ID_usu=usuarios.ID_usu AND obras.ID_zon=zonas.ID_zon AND registro_servicio.ser_asig='". $ser_asig . "' ORDER BY registro_servicio.ser_cod DESC";
		 $result	=	mysql_query($sql);
		 return $result;
	 }

	 function registro_servicio_id($ID_ser){
		 $sql		=	"SELECT * FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas WHERE registro_servicio.ID_ser='" . $ID_ser . "' AND registro_servicio.ID_cli=clientes.ID_cli AND registro_servicio.ID_obr=obras.ID_obr AND registro_servicio.ID_sta=status.ID_sta AND registro_servicio.ID_pri=prioridades.ID_pri AND registro_servicio.ID_usu=usuarios.ID_usu AND obras.ID_zon=zonas.ID_zon";
		 $result	=	mysql_query($sql);
		 return $result;
	 }
	...
	function insert_registro_servicio($ID_cli, $ID_obr, $ID_sta, $ID_pri, $ID_usu, $ser_tipo, $ser_cod, $ser_asig, $ser_desc, $ser_contacto, $ser_telefono, $ser_mail, $ser_fecin, $ser_hourin){
		$sql		= "INSERT INTO registro_servicio (ID_cli, ID_obr, ID_sta, ID_pri, ID_usu, ser_tipo, ser_cod, ser_asig, ser_desc, ser_contacto, ser_telefono, ser_mail, ser_fecin, ser_hourin) VALUES('" . $ID_cli . "','" . $ID_obr . "','" . $ID_sta . "','" . $ID_pri . "','" . $ID_usu . "', '" . $ser_tipo . "', '" . $ser_cod . "','" . $ser_asig . "','" . $ser_desc . "','" . $ser_contacto . "','" . $ser_telefono . "','" . $ser_mail . "','" . $ser_fecin . "','" . $ser_hourin . "')";
		$result		= mysql_query($sql);
		if($result) {return 1;} else { return 0;};
	}

	function update_registro_servicio($ID_ser, $ID_cli, $ID_obr, $ID_sta, $ID_pri, $ID_usu, $ser_tipo, $ser_asig, $ser_desc, $ser_contacto, $ser_telefono, $ser_mail, $ser_fecin, $ser_hourin, $ser_recCli){
	...
	function registro_servicio_cierra($ID_ser, $ser_solucion, $ser_persconforme, $ser_conforme, $ser_hs, $ser_costo, $ser_fecout, $ser_hourout, $ser_cerrado){
		$sql		= "UPDATE registro_servicio SET ID_sta='11', ser_solucion='" . $ser_solucion . "', ser_persconforme='" . $ser_persconforme . "', ser_conforme='" . $ser_conforme . "', ser_hs='" . $ser_hs . "', ser_costo='" . $ser_costo . "', ser_fecout='" . $ser_fecout . "', ser_hourout='" . $ser_hourout . "', ser_cerrado='" . $ser_cerrado . "' WHERE ID_ser='" . $ID_ser  ."'";
		$result		= mysql_query($sql);
		if($result) {return 1;} else { return 0;};
	}
...
//////////////// CONSULTAS PARA LA TABLA REGISTRO SERVICIOS
	 ...
	 function registro_presupuesto_nextid(){
		 $sql		=	"SELECT AUTO_INCREMENT AS NEXTID FROM information_schema.tables WHERE table_name = 'registro_servicio' AND table_schema='travel_expenses'";
		 $result	=	mysql_query($sql);
		 return $result;
	 }
	...
	function registro_presupuesto_rechazado($ID_ser, $ID_mot){
	 ...
	 function registro_presupuesto_pendientes_instala($ID_usu){
		...
		$sql		=	"SELECT *, TIMESTAMPDIFF(DAY, registro_servicio.ser_fecDespacho, CURDATE()) AS days FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas, provincias, ciudades 
		 				WHERE
	 ...
	 function registro_presupuesto_pendientes_cierre($ID_usu){
		...
		$sql		=	"SELECT *, TIMESTAMPDIFF(DAY, registro_servicio.ser_fecDespacho, CURDATE()) AS days FROM registro_servicio, obras, clientes, status, prioridades, usuarios, zonas, provincias, ciudades, tipo_presupuesto
		 				WHERE
...
//////////////// CONSULTAS PARA TASKS
	  ...
	  function registro_servicio_id_tas($ID_tas){
...
//////////////// CONSULTAS PARA CONTRATISTAS Y SUPERVISORES Y VENDEDORES
	 ...
	 function contratistas(){
	  ...
	  function contratistasSupervisores(){
		 $sql		=	"SELECT * FROM usuarios WHERE ID_tpu='22' or ID_tpu='5' ORDER BY usu_apellido ASC";
		 $result	=	mysql_query($sql);
		 return $result;
	 }
