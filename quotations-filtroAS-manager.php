...
{
         $sql = "SELECT *
              FROM quotations WHERE pto_pedidoCod=".$codigo."";
...
if($contA==1)
  {
        if($contB==1 or $contC==1 or $contD==1 or $contF==1 or $contE==1 or $contG==1)
    {
  ...
  if($contB==1)
  {
    if($contC==1 or $contD==1 or $contF==1 or $contE==1 or $contG==1)
    {
    ...
    if($contG==1)
  {
    ...
    {
      $filtro_vendedor = $filtro_vendedor . " AND";
    }
  }
  ...
  { 
    ...
    // echo "SELECT * FROM quotations ".$where." ".$filtro_usuario." ".$filtro_asignado." ".$filtro_vendedor."  ".$filtro_cliente."  ".$filtro_tienda." ".$filtro_estado." ".$desde." ";
        ...
        $sql = "SELECT *,
             ...
             DATE_FORMAT(pto_fecBloqueo, '%d-%m-%Y ' ' %H:%i:%s') as pto_fecBloqueo,
              ...
              DATE_FORMAT(pto_fecInstalacion, '%d-%m-%Y ' ' %H:%i:%s') as pto_fecInstalacion  FROM quotations ".$where." ".$filtro_usuario." ".$filtro_asignado." ".$filtro_vendedor." ".$filtro_cliente."  ".$filtro_tienda." ".$filtro_estado."  
         ".$desde." AND ID_tpp!=6";
        $result = mysql_query($sql);
             $num_result = mysql_num_rows($result);
            }
